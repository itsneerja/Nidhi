...
@Nidhi.on(events.NewMessage(pattern="^[!?/]kick ?(.*)"))
@is_admin
async def kick(event, perm):
    ...
    info = await Nidhi.get_entity(us)
    await Nidhi.kick_participant(event.chat_id, input_str or replied_user)
    await event.reply(f"sᴜᴄᴄᴇssғᴜʟʟʏ ᴋɪᴄᴋᴇᴅ [{info.first_name}](tg://user?id={replied_user}) ғʀᴏᴍ {event.chat.title}")

@Nidhi.on(events.NewMessage(pattern="^[!?/]kickme"))
async def kickme(event):
    ...
    check = await Nidhi.get_permissions(event.chat_id, event.sender_id)
    if check.is_admin:
        await event.reply("sᴏʀʀʏ ʙᴜᴛ ɪ ᴄᴀɴ'ᴛ ᴋɪᴄᴋ ᴀᴅᴍɪɴs.")
        return

    await event.reply("ᴏᴋ, ᴀs ʏᴏᴜʀ ᴡɪsʜ")
    await Nidhi.kick_participant(event.chat_id, event.sender_id)

@Nidhi.on(events.NewMessage(pattern="^[!?/]ban ?(.*)"))
@is_admin
async def ban(event, perm):
    ...
    us = msg.sender.username
    info = await Nidhi.get_entity(us)
    await Nidhi(EditBannedRequest(event.chat_id, replied_user, ChatBannedRights(until_date=None, view_messages=True)))
    await event.reply(f"sᴜᴄᴄᴇssғᴜʟʟʏ ʙᴀɴɴᴇᴅ [{info.first_name}](tg://user?id={replied_user}) ɪɴ {event.chat.title}")

@Nidhi.on(events.NewMessage(pattern="^[!?/]unban ?(.*)"))
@is_admin
async def unban(event, perm):
    ...
    us = msg.sender.username
    info = await Nidhi.get_entity(us)
    await Nidhi(EditBannedRequest(event.chat_id, replied_user, ChatBannedRights(until_date=None, view_messages=False)))
    await event.reply(f"sᴜᴄᴄᴇssғᴜʟʟʏ ᴜɴʙᴀɴɴᴇᴅ [{info.first_name}](tg://user?id={replied_user}) ɪɴ {event.chat.title}")

@Nidhi.on(events.NewMessage(pattern="^[!?/]skick"))
@is_admin
async def skick(event, perm):
    ...
    await event.delete()
    await Nidhi.kick_participant(event.chat_id, x)
    await event.reply(f"sᴜᴄᴄᴇssғᴜʟʟʏ ᴋɪᴄᴋᴇᴅ [{info.first_name}](tg://user?id={replied_user}) ғʀᴏᴍ {event.chat.title}")

@Nidhi.on(events.NewMessage(pattern="^[!?/]dkick"))
@is_admin
async def dkick(event, perm):
    ...
    await x.delete()
    await Nidhi.kick_participant(event.chat_id, x.sender_id)
    await event.reply(f"sᴜᴄᴄᴇssғᴜʟʟʏ ᴋɪᴄᴋᴇᴅ [{info.first_name}](tg://user?id={replied_user}) ғʀᴏᴍ {event.chat.title}")

@Nidhi.on(events.NewMessage(pattern="^[!?/]dban"))
@is_admin
async def dban(event, perm):
    ...
    zx = (await event.get_reply_message())
    await zx.delete()
    await Nidhi(EditBannedRequest(event.chat_id, x, ChatBannedRights(until_date=None, view_messages=True)))
...
@Nidhi.on(events.NewMessage(pattern="^[!?/]sban"))
@is_admin
...
@Nidhi.on(events.callbackquery.CallbackQuery(data="bans"))
