...
@Nidhi.on(events.NewMessage(pattern="^[!?/]id"))
async def id(event):
...
@Nidhi.on(events.NewMessage(pattern="^[!?/]info ?(.*)"))
async def info(event):
    if Config.MANAGEMENT_MODE == "ENABLE":
        return
    sed = await Nidhi(P(user_id=event.sender_id, offset=42, max_id=0, limit=80))
    hn = await Nidhi(GetFullUserRequest(event.sender_id))
    text = "**» ᴜsᴇʀ ɪɴғᴏ:**\n\n"
    text += "**» ғɪʀsᴛ ɴᴀᴍᴇ:** {}\n"
    text += "**» ʟᴀsᴛ ɴᴀᴍᴇ:** {}\n"
    text += "**» ᴜsᴇʀ-ɪᴅ:** `{}`\n"
    ...
    if not input_str:
          await Nidhi.send_message(event.chat_id, text.format(hn.user.first_name, hn.user.last_name, event.sender_id, event.sender.username, sed.count, hn.about, event.sender_id))
          return
 
    input_str = event.pattern_match.group(1)
    ha = await Nidhi.get_entity(input_str)
    hu = await Nidhi(GetFullUserRequest(id=input_str))
    sedd = await Nidhi(P(user_id=input_str, offset=42, max_id=0, limit=80))
...
@Nidhi.on(events.callbackquery.CallbackQuery(data="misc"))
async def _(event):
